# Orbbec Gemini 335L + YOLOv8 单人跟踪系统说明

本项目通过 Orbbec Gemini 335L 相机 + YOLOv8 + KCF + ReID + 卡尔曼滤波，实现「只追踪同一个人」的稳健跟踪，并可视化真实轨迹与预测轨迹。
...
```mermaid
flowchart TD
    A[启动程序] --> B[初始化 Orbbec Gemini 335L 彩色+深度流]
    B --> C[加载 YOLOv8 模型]
...
### 8. Orbbec Gemini 335L 深度估计

- 使用 `pyorbbecsdk` 获取深度帧，并在当前 ROI 内统计有效深度（去零值、百分位裁剪异常值、求平均）。
...
- 运行环境：
  - Python 3.x
  - 主要依赖：`pyorbbecsdk`, `opencv-python`, `numpy`, `ultralytics`, `torch`, `torchvision`, `scipy` 等。
- 运行方式（在虚拟环境或系统环境中）：
...
以下是程序中各个阈值参数的详细解释，包括其作用、调大/调小的影响。这些参数经过实测优化，适合一般场景。调参时建议一次只改一个参数，并在不同环境下测试。

### 1. YOLO_PERSON_CONF = 0.45 (YOLO人体检测置信度)
   - **作用**：控制YOLO模型检测人体时的最低置信度阈值。只有置信度高于此值的检测框才被认为是有效的人体。
   - **调大**：减少误检，但可能错过低置信度的真实目标，导致跟踪丢失或延迟启动。
   - **调小**：增加检测到的潜在目标，但可能引入噪声，导致跟踪不稳定。

### 2. REID_SIM_THRESHOLD = 0.55 (自适应外观特征的ReID阈值)
   - **作用**：用于ReID时判断两个外观特征是否匹配的相似度阈值。
   - **调大**：提高ReID严格性，减少误跟，但可能在外观变化时丢失目标。
   - **调小**：放宽ReID条件，更容易重获目标，但增加误跟风险。

### 3. BASE_SIM_THRESHOLD = 0.55 (初始外观模板相似度阈值)
   - **作用**：初始跟踪目标的外观模板与候选检测框的相似度阈值。
   - **调大**：更严格的初始匹配，减少误跟，但启动跟踪更难。
   - **调小**：更容易匹配初始目标，跟踪启动更快，但可能一开始跟错。

### 4. COLOR_SIM_THRESHOLD = 0.60 (衣服颜色HSV直方图相似度阈值)
   - **作用**：基于衣服颜色的辅助相似度阈值，避免跟错不同颜色的人。
   - **调大**：强调颜色匹配，适用于颜色差异明显的场景。
   - **调小**：减少颜色约束，更依赖外观特征。

### 5. RELAXED_SCORE_THRESHOLD = 0.50 (丢失后ReID重获时的放宽条件)
   - **作用**：目标丢失后重获时的最低相似度阈值。
   - **调大**：重获更严格，减少误重获。
   - **调小**：更容易重获丢失目标，跟踪更连续。

### 6. RELAXED_BASE_DELTA = 0.10 (BASE_SIM_THRESHOLD可放宽的幅度)
   - **作用**：在放宽条件下，BASE_SIM_THRESHOLD最多可放宽的量。
   - **调大**：允许更大的阈值放宽，重获更容易。
   - **调小**：放宽幅度有限，重获更保守。

### 7. RELAXED_COLOR_DELTA = 0.20 (COLOR_SIM_THRESHOLD可放宽的幅度)
   - **作用**：类似RELAXED_BASE_DELTA，但针对颜色相似度。
   - **调大**：颜色匹配更宽松。
   - **调小**：颜色匹配更严格。

### 8. RELAXED_SCORE_MARGIN = 0.05 (最佳候选比第二高至少高出的margin)
   - **作用**：重获时，最佳候选必须比第二高高出至少此margin。
   - **调大**：确保最佳候选明显优于其他人。
   - **调小**：允许相似度差距较小的情况重获。

### 9. MAX_LOST_FRAMES = 10 (YOLO检测丢失帧数阈值)
   - **作用**：目标连续多少帧未被检测到，才认为真正丢失。
   - **调大**：容忍更长的临时遮挡，跟踪更稳定。
   - **调小**：快速响应丢失，适用于快速运动场景。

### 10. TARGET_DISTANCE = 1.5 (期望跟随距离，单位m)
   - **作用**：PID控制器试图保持的理想跟随距离。
   - **调大**：保持更远距离，跟随更保守。
   - **调小**：更近距离跟随，更亲近但风险更高。

### 11. MAX_TRACK_LOST = 30 (连续丢失帧数上限)
   - **作用**：连续丢失超过此帧数，停止跟踪。
   - **调大**：容忍更长的丢失时间。
   - **调小**：快速放弃跟踪。

### 12. IOU_MATCH_THRESH = 0.3 (初始ROI与检测框匹配阈值)
   - **作用**：鼠标选择的初始ROI与YOLO检测框的IoU阈值。
   - **调大**：要求更精确的匹配。
   - **调小**：允许更宽松的匹配。

### 13. CLUSTER_EPS_M = 0.6 (DBSCAN风格聚类半径，单位m)
   - **作用**：在X-Z平面上聚类检测到的多个人的距离阈值。
   - **调大**：将距离较远的人聚在一起。
   - **调小**：只选择非常近的人。

### 14. YOLO_DETECT_INTERVAL = 1 (YOLO每帧检测间隔)
   - **作用**：每多少帧运行一次YOLO检测。
   - **调大**：减少YOLO调用，提升性能，但精度下降。
   - **调小**：每帧检测，精度最高但负载大。

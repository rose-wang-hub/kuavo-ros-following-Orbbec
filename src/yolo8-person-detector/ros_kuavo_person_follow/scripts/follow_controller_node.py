...
class PIDFollower(object):
    ...
    def compute(self, state):
        ...
        # 自定义速度逻辑：距离2.5米内，越近速度越慢；2.5米外最大速度
        if state.z < 2.5:
            v_cmd = self.v_max * (state.z / 2.5)
        else:
            v_cmd = self.v_max
        ...
        # 如果转弯角度超过15度，降低前进速度到0.1，并慢转向
        if abs(yaw_err) > math.radians(15):
            v_cmd = 0.1
            yaw_rate_cmd *= 0.5  # 减慢转向速度

        v_cmd = max(0.0, min(self.v_max, v_cmd))  # 确保非负
...
class FollowControllerNode(object):
    def __init__(self):
        ...
        self.state_timeout = rospy.get_param('~state_timeout', 0.1)
        self._last_state_time = None
        self._watchdog_timer = rospy.Timer(rospy.Duration(0.05), self._watchdog_cb)

        # 程序退出时发布停止命令，确保机器人停止运动
    ...
    def _shutdown_cb(self):
        """程序退出时发布停止命令，确保机器人完全停止运动。"""
        cmd = Twist()
        # 发布多次确保下位机接收到停止命令
        for _ in range(5):
            self.pub.publish(cmd)
            time.sleep(0.01)  # 短暂延迟
        rospy.loginfo('follow_controller_node 已发布停止命令')
